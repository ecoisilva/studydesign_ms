...
This document describes in detail how to run the workflow presented in the manuscript, ***"Too few, too many, or just right? Optimizing sample sizes for population-level inferences in animal tracking projects"***, using the `movedesign` `R` package and Shiny application [@silva2023movedesign]. Preprint will be available on **BioRxiv** soon. For any definitions, check the main manuscript or the previous publication associated with this `R` package [@silva2023movedesign]. You will first need to install `movedesign`:
...
We will outline the process by which we assess the impact of different sampling designs on common movement ecology metrics, *i.e.*, home range area and movement speed, for population-level inferences. All code has been executed in `R` version `r getRversion()` (R. Core Team, 2025), `ctmm` version `r packageVersion("ctmm")`, and `movedesign` version `r packageVersion("movedesign")`.
...
library(movedesign)
movedesign::run_app() # will open the app


```

This workflow is a simplified case study, designed for quick execution. We will use a GPS tracking dataset of African buffalos (*Syncerus caffer*) tracked in Kruger National Park between 2005 and 2006 [@cross2009], to inform our simulations. Our primary goal is to reliably estimate **mean home range area** of a population of African Buffalos.

Similarly to the [single estimate](https://ecoisilva.github.io/movedesign/articles/movedesign.html) tutorial, we begin by selecting the appropriate workflow in the `r fa("house")` `Home` tab. Please check that vignette, as some information is omitted here to avoid repetition. In this tutorial, we will showcase a workflow for **deploying a set number of units**. 

![](ext_images/tutorial2_img1.png){fig-align="center"} 

<!-- ::: {.callout-tip icon=false} -->
<!-- ## `r fontawesome::fa("bell", fill = "#ffffff")` Instructions -->
<!-- ::: -->
...
![](ext_images/tutorial2_img2.png){fig-align="center"}
...
![](ext_images/tutorial2_img3.png){fig-align="center"}
...
![](ext_images/tutorial2_img4.png){fig-align="center"}
...
![](ext_images/tutorial2_img5.png){fig-align="center"}
...
![](ext_images/tutorial2_img6.png){fig-align="center"}
...
![](ext_images/tutorial2_img7.png){fig-align="center"}
...
![](ext_images/tutorial2_img8.png){fig-align="center"}
