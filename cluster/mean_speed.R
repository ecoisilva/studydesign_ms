...
rv$simfitList <- fitting_model(rv)
...
saveRDS(list(sim = rv$simList,
        ...
        file = here::here("data", "processed",
                          paste0(rv$species,
                                 "_dur", rv$dur$value, rv$dur$unit,
                                 "_dti", rv$dti$value, rv$dti$unit,
                                 "_", individuals_no, "inds",
                                 "_simfitList.rds")))
gc()

message("Estimating speed & distance...")
rv$ctsdList <- estimate_speed(rv)
print(Sys.time() - start_total)
